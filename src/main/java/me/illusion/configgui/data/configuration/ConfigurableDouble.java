...
public class ConfigurableDouble implements Configurable {
    ...
    public ConfigurableDouble(double val, ConfigurationPlugin main, String path, File file, FileConfiguration config, int slot, Menu menu, Menu previous)
    {
        this.menu = menu;
        ...
        item = new ItemBuilder(Material.SKULL_ITEM)
                ...
                .lore("",
                        "&bLeft Click &7to increase by 0.1",
                        "&bShift Left click &7to increase by 1",
                        "&cRight Click &7to decrease by 0.1",
                        "&cShirt Right Click &7to decrease by 0.1",
                        "",
                        "&7Current Value: &6" + val)
                .build();

        menu.setItem(slot, item, (event) -> {
            if (event.isLeftClick())
                this.val = this.val + (event.isShiftClick() ? 1 : 0.1);
    ...
    public boolean setValue(Object value) {
        ...
        {
            boolean allowed = Doubles.tryParse((String) value) == null;
            if(allowed)
